...
Parse `$ARGUMENTS`: Ticket ID → fetch + look for plan in comments · Text → direct instructions · None → use plan from this session
...
- **Session plan** (from earlier planning) → use it
...
- **Insufficient info** (ticket too vague, no actionable steps, no argument) → ask:
  ...
  Which approach?
  ```
  If user picks 1: run the planning phase inline (same logic as `/br-plan`), then continue to step 2 after user confirms the plan.
